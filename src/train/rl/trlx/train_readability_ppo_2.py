...
model_dir = '/home/cunhuan/code/controllable-readability-summarization/src/train/mnt/hd3/checkpoints/summary'
...
config = TRLConfig(
    train=TrainConfig(
        ...
        checkpoint_dir='checkpoint-diverse-readability-2-phases',
        save_best=True
    ),
    model=ModelConfig(
...
if __name__ == "__main__":
    def reward_fn(samples: List[str], prompts: List[str], outputs: List[str], tokenizer=None):
        ...
        global_step_counter += 1  # Increment step counter for each reward function call
        ...
        # Switch weights after approximately 2 epochs
        switch_step = config.train.total_steps // 5
        if global_step_counter < switch_step:
            readability_weight = 0.8
            bertscore_weight = 0.2
            word_level_weight = 0.0
        else:
            readability_weight = 0.0
            bertscore_weight = 0.2
            word_level_weight = 0.8
        ...
        final_scores = readability_weight * flesch_scores + bertscore_weight * all_bertscore_scores + word_level_weight * word_level_probs
