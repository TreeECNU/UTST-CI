...
model_dir = '/home/cunhuan/code/controllable-readability-summarization/src/train/mnt/hd3/checkpoints/exec-10000/checkpoint-150000'  # select the checkpoint from the prompt-based methods
...
if __name__ == "__main__":
    ...
    # def reward_fn(samples: List[str], prompts: List[str], outputs: List[str], tokenizer=None, iter_count=0):
    #     # 使用全局的 tokenizer 和 model，而不是传入的 tokenizer 参数
    #     local_model_name = "bert-large"
    #     local_model = AutoModelForMaskedLM.from_pretrained(local_model_name).to(device)  # 使用 AutoModelForMaskedLM
    ...
    #         try:
    #             inputs = local_tokenizer(generated_summary, return_tensors="pt", padding=True, truncation=True, max_length=512).to(device)
    #             input_ids = inputs["input_ids"].clone()
    #             attention_mask = inputs["attention_mask"]

    #             mask_prob = 0.15
    #             rand = torch.rand(input_ids.shape).to(device)
    #             mask = (rand < mask_prob) & (attention_mask == 1)  # 只掩码非填充 token
    #             labels = input_ids.clone()
    #             input_ids[mask] = local_tokenizer.mask_token_id  # 用 [MASK] 替换

    #             with torch.no_grad():
    #                 outputs = local_model(
    #                     input_ids=input_ids,
    #                     attention_mask=attention_mask,
    #                     labels=labels  # RoBERTa 的 MLM 支持 labels
    #                 )
    #                 loss = outputs.loss.item()
    #                 ppl = math.exp(loss)  # 将 MLM 损失转换为“困惑度”
    #             ppl_scores.append(ppl)
    #         except Exception as e:
    #             print(f"PPL calculation error for summary '{generated_summary}': {e}")
    #             ppl_scores.append(50.0)
    #     # 计算 BERTScore
    #     all_bertscore_scores = []
    #     for doc, summary in zip(docs, summaries):
    #         bertscore_input_embedding, bertscore_input_attention_mask, bertscore_input_idf = encode_text([doc])
    ...
    #     ppl_scores = [1.0 - (ppl / max_ppl) if max_ppl > 0 else 1.0 for ppl in ppl_scores]
