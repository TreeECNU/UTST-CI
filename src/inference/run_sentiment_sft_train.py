...
def main():
    ...
    sentiment_model_name = "YELP-Review_Classifier"  # Replace with your model name if different
    try:
        sentiment_model = AutoModelForSequenceClassification.from_pretrained(sentiment_model_name)
        sentiment_tokenizer = AutoTokenizer.from_pretrained(sentiment_model_name)
    except Exception as e:
        logger.warning(f"Failed to load sentiment classifier {sentiment_model_name}: {e}. Sentiment analysis will be skipped.")
    ...
    def compute_metrics(eval_preds):
        ...
        # Compute sentiment analysis and probability balance
        balance_results = []
        balance_threshold = 0.3  # Threshold for max-min probability difference (adjustable)
        
        if sentiment_model is not None and sentiment_tokenizer is not None:
            for pred in decoded_preds:
                inputs = sentiment_tokenizer(pred, return_tensors="pt", padding=True, truncation=True)
                with torch.no_grad():
                    outputs = sentiment_model(**inputs)
                logits = outputs.logits
                # Convert logits to probabilities
                probabilities = softmax(logits.numpy(), axis=-1)[0]  # Shape: (5,)
                # Check if probabilities are balanced
                max_prob = np.max(probabilities)
                min_prob = np.min(probabilities)
                is_balanced = (max_prob - min_prob) < balance_threshold
            ...
            logger.warning("Sentiment classifier not loaded. Skipping balance check.")
            balance_results = [{"input_text": pred, "probabilities": [0.2] * 5, "is_balanced": False} for pred in decoded_preds]

        # Save balance results to a CSV file
        if trainer.is_world_process_zero():
            output_balance_file = os.path.join(training_args.output_dir, "sentiment_balance_results.csv")
            with open(output_balance_file, "w", newline="", encoding="utf-8") as writer:
                csv_writer = csv.writer(writer)
                csv_writer.writerow(["Input Text", "1-Star", "2-Star", "3-Star", "4-Star", "5-Star", "Is Balanced"])
                for res in balance_results:
                    probs = res["probabilities"]
                    # Truncate input text to avoid CSV formatting issues
                    input_text = res["input_text"].replace("\n", " ")[:100]
                    csv_writer.writerow([input_text, probs[0], probs[1], probs[2], probs[3], probs[4], res["is_balanced"]])

        # Return empty result since no other metrics are computed
        return {}
