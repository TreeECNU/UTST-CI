...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        # Compute sentiment analysis for generated summaries
        sentiment_results = []
        if sentiment_model is not None and sentiment_tokenizer is not None:
            for pred in decoded_preds:
                ...
                sentiment_results.append(prediction)  # Store numerical star rating (0-4)
        else:
            logger.warning("Sentiment classifier not loaded. Skipping sentiment analysis.")
            sentiment_results = [0] * len(decoded_preds)  # Fallback to neutral rating

        # Compute average star rating (1-5 stars)
        avg_sentiment = np.mean([rating + 1 for rating in sentiment_results]) if sentiment_results else 0  # Convert 0-4 to 1-5
        sentiment_result = {"avg_star_rating": round(avg_sentiment, 4)}
        ...
        # result.update(bertscore_result)  # Commented out as in original code
        # result.update(readability_result)  # Commented out as in original code
